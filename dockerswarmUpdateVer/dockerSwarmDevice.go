...
func (dockerSwarmDevice) SetImages(appName string, dockerImages string, progress chan string, ctx context.Context) bool {
	...
	var exitCode = exec.RunShellContext(ctx, fmt.Sprintf("docker service update --image %s --update-delay 10s --with-registry-auth %s", dockerImages, appName), progress, nil, "", true)
	if exitCode != 0 {
